...
public class HerbalismCraftingStationRecipe extends AbstractWorkstationRecipe {
    public HerbalismCraftingStationRecipe() {
        ...
        addIngredientBehaviour(new ConsumeHerbIngredientBehaviour(herbComponentPredicate, 1,
    ...
    public HerbalismCraftingStationRecipe(String prefabPath, String displayName, float requiredTemperature,
    ...
    public HerbalismCraftingStationRecipe(String prefabPath, String displayName, List<String> recipeComponents,
                                          float requiredTemperature, long processingDuration) {
        Predicate<EntityRef> herbComponentPredicate = new Predicate<EntityRef>() {
            @Override
            public boolean apply(EntityRef input) {
                return input.hasComponent(HerbComponent.class);
        ...
        for (String component : recipeComponents) {
            ...
            String type = split[1];
            addIngredientBehaviour(new ConsumeHerbIngredientBehaviour(herbComponentPredicate, count,
                    new InventorySlotTypeResolver("INPUT")));
        }
    ...
    public HerbalismCraftingStationRecipe(CraftingStationRecipeComponent recipe) {
        ...
        for (String component : recipe.recipeComponents) {
            ...
            //addIngredientBehaviour(new ConsumeHerbIngredientBehaviour(herbComponentPredicate, count, new 
            // InventorySlotTypeResolver("INPUT")));

            addIngredientBehaviour(new ConsumeHerbIngredientBehaviour(new HerbalismStationIngredientPredicate(type),
                    count, new InventorySlotTypeResolver("INPUT")));
        }

        addFluidBehaviour(new ConsumeFluidBehaviour("Fluid:Water", 0.2f, new InventorySlotTypeResolver("FLUID_INPUT")));
        setRequiredHeat(recipe.requiredTemperature);
        setProcessingDuration(recipe.processingDuration);
        setResultFactory(new PotionRecipeResultFactory(Assets.getPrefab(recipe.recipeId).get(),
                recipe.itemResult.split("\\*")[1], 1));
    }
    ...
    private final class ConsumeHerbIngredientBehaviour extends ConsumeItemCraftBehaviour {
        private ConsumeHerbIngredientBehaviour(Predicate<EntityRef> matcher, int count,
        ...
        protected String getParameter(List<Integer> slots, EntityRef item) {
            ...
            final String herbName = CoreRegistry.get(GenomeManager.class).getGenomeProperty(item,
                    Herbalism.NAME_PROPERTY, String.class);
