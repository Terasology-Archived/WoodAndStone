...
package org.terasology.woodandstone.milling.processPart;
...
import org.terasology.woodandstone.milling.component.MillProcessedComponent;
import org.terasology.woodandstone.milling.component.MillProgressComponent;
...
public class MillProcessingProcessPartCommonSystem extends BaseComponentSystem {
        ...
        if (event.getWorkstation().hasComponent(WorkstationInventoryComponent.class)) {
            ...
            for (int slot : WorkstationInventoryUtils.getAssignedSlots(event.getWorkstation(), "INPUT")) {
                MillProcessedComponent processed =
                        InventoryUtils.getItemAt(event.getWorkstation(), slot).getComponent(MillProcessedComponent.class);
                if (processed != null) {
    ...
    public void startExecution(ProcessEntityStartExecutionEvent event, EntityRef processEntity,
        ...
        if (!event.getWorkstation().hasComponent(MillProgressComponent.class)) {
            ...
            final EntityRef removedItem = inventoryManager.removeItem(event.getWorkstation(), event.getInstigator(),
                    item, false, 1);
            if (removedItem != null) {
    ...
    public void getDuration(ProcessEntityGetDurationEvent event, EntityRef processEntity,
        ...
        if (progress != null) {
            ...
            processed =
                    InventoryUtils.getItemAt(event.getWorkstation(), component.slot).getComponent(MillProcessedComponent.class);
        }
    ...
    public void finishExecution(ProcessEntityFinishExecutionEvent event, EntityRef entityRef,
        ...
        if (millProgress.processedStep < MILL_STEP_COUNT) {
            ...
            if (inventoryManager.giveItem(event.getWorkstation(), event.getInstigator(), resultItem,
