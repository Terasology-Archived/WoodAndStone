...
import org.terasology.woodandstone.ui.HerbalismCraftingStationRecipe;
...
public class RegisterWoodAndStoneRecipes extends BaseComponentSystem {
    ...
    public void initialise() {
        workstationRegistry.registerProcessFactory(WoodAndStone.BASIC_WOODCRAFTING_PROCESS_TYPE,
                new CraftingWorkstationProcessFactory());
        workstationRegistry.registerProcessFactory(WoodAndStone.ADVANCED_WOODCRAFTING_PROCESS_TYPE,
                new CraftingWorkstationProcessFactory());

        workstationRegistry.registerProcessFactory(WoodAndStone.BASIC_STONECRAFTING_PROCESS_TYPE,
                new CraftingWorkstationProcessFactory());
        workstationRegistry.registerProcessFactory(WoodAndStone.ADVANCED_STONECRAFTING_PROCESS_TYPE,
                new CraftingWorkstationProcessFactory());

        workstationRegistry.registerProcessFactory(WoodAndStone.COOKING_PROCESS_TYPE,
                new CraftingWorkstationProcessFactory());
        workstationRegistry.registerProcessFactory(WoodAndStone.HERBALISM_PROCESS_TYPE,
                new CraftingWorkstationProcessFactory());
    ...
    private void addWoodPlankRecipes() {
        workstationRegistry.registerProcess(WoodAndStone.BASIC_WOODCRAFTING_PROCESS_TYPE,
                new CraftingWorkstationProcess(WoodAndStone.BASIC_WOODCRAFTING_PROCESS_TYPE, "Materials|WoodAndStone" +
                        ":WoodPlank",
                        new PlankRecipe(2)));

        workstationRegistry.registerProcess(WoodAndStone.ADVANCED_WOODCRAFTING_PROCESS_TYPE,
                new CraftingWorkstationProcess(WoodAndStone.ADVANCED_WOODCRAFTING_PROCESS_TYPE, "Materials" +
                        "|WoodAndStone:WoodPlank",
                        new PlankRecipe(3)));
    }
    ...
    private void addWorkstationFormingRecipes() {
        multiBlockFormRecipeRegistry.addMultiBlockFormItemRecipe(
                new UniformMultiBlockFormItemRecipe(new ToolTypeEntityFilter("axe"), new UseOnTopFilter(),
                        ...
                        new UniformBlockReplacementCallback<Void>(blockManager.getBlock("WoodAndStone" +
                                ":BasicWoodStation"))));
        multiBlockFormRecipeRegistry.addMultiBlockFormItemRecipe(
        ...
        herbalismStationRecipe.addLayer(1, 1, new BlockUriEntityFilter(new BlockUri(new ResourceUrn("CoreAssets" +
                ":CobbleStone"), new ResourceUrn(("Engine:EighthBlock")))));
    ...
    private void addHerbalismWorkstationRecipes() {
        ...
        /* workstationRegistry.registerProcess(WoodAndStone.HERBALISM_PROCESS_TYPE,
                new CraftingWorkstationProcess(WoodAndStone.HERBALISM_PROCESS_TYPE, "WoodAndStone:HerbPotion", new 
                HerbalismCraftingStationRecipe()));*/
    ...
    private void addStandardWoodWorkstationBlockShapeRecipes() {
        ...
        addWorkstationBlockShapesRecipe(WoodAndStone.ADVANCED_WOODCRAFTING_PROCESS_TYPE, "Building|Fine " +
                        "Planks|WoodAndStone:FinePlankBlock",
                "WoodAndStone:plank", 4, "hammer", 1, "WoodAndStone:FinePlank", 1);
    }

    private void addBasicStoneWorkstationBlockShapeRecipes() {
        addWorkstationBlockShapesRecipe(WoodAndStone.BASIC_STONECRAFTING_PROCESS_TYPE, "Building|Cobble " +
                        "Stone|WoodAndStone:CobbleBlock",
                "WoodAndStone:stone", 2, "hammer", 1, "CoreAssets:CobbleStone", 1);
        addWorkstationBlockShapesRecipe(WoodAndStone.ADVANCED_STONECRAFTING_PROCESS_TYPE, "Building|Bricks" +
                        "|WoodAndStone:BrickBlock",
                "WoodAndStone:brick", 2, "hammer", 1, "CoreAssets:Brick", 1);
    }

    private void addPlankBlockShapeRecipe(String shape, int ingredientMultiplier, int durabilityMultiplier,
                                          int resultMultiplier) {
        addPlankBlockShapeRecipe(shape, "Engine", ingredientMultiplier, durabilityMultiplier, resultMultiplier);
    }

    private void addPlankBlockShapeRecipe(String shape, String module, int ingredientMultiplier,
                                          int durabilityMultiplier, int resultMultiplier) {
        ...
        workstationRegistry.registerProcess(WoodAndStone.ADVANCED_WOODCRAFTING_PROCESS_TYPE,
                new CraftingWorkstationProcess(WoodAndStone.ADVANCED_WOODCRAFTING_PROCESS_TYPE, recipeName,
                        new PlankBlockRecipe(2 * ingredientMultiplier, durabilityMultiplier, resultShape,
                                4 * resultMultiplier)));
    }
    ...
    private void addShapeRecipe(String processType, String recipeNamePrefix, String ingredient,
                                int ingredientBasicCount,
                                String tool, int toolDurability, String blockResultPrefix, int blockResultCount,
                                String shape, int ingredientMultiplier, int resultMultiplier,
                                int toolDurabilityMultiplier) {
        addShapeRecipe(processType, recipeNamePrefix, ingredient, ingredientBasicCount, tool, toolDurability,
                blockResultPrefix, blockResultCount, shape,
    ...
    private void addShapeRecipe(String processType, String recipeNamePrefix, String ingredient,
                                int ingredientBasicCount,
                                String tool, int toolDurability, String blockResultPrefix, int blockResultCount,
                                String shape, String module, int ingredientMultiplier, int resultMultiplier,
                                int toolDurabilityMultiplier) {
        ...
        workstationRegistry.registerProcess(processType, new CraftingWorkstationProcess(processType,
                recipeNamePrefix + shape, shapeRecipe));
    }

    private void addWorkstationBlockShapesRecipe(String processType, String recipeNamePrefix, String ingredient,
                                                 int ingredientBasicCount,
                                                 String tool, int toolDurability, String blockResultPrefix,
                                                 int blockResultCount) {
        ...
        workstationRegistry.registerProcess(processType, new CraftingWorkstationProcess(processType, recipeNamePrefix
                , fullBlockRecipe));

        addShapeRecipe(processType, recipeNamePrefix, ingredient, ingredientBasicCount, tool, toolDurability,
                blockResultPrefix, blockResultCount,
                "Stair", 3, 4, 2);
    ...
    private void parseCraftInHandRecipe(CraftInHandRecipeComponent recipeComponent) {
        ...
        if (recipeComponent.blockResult != null) {
            resultFactory =
                    new BlockRecipeResultFactory(blockManager.getBlockFamily(recipeComponent.blockResult).getArchetypeBlock(), 1);
        } else {
        ...
        if (recipeComponent.recipeComponents != null) {
            for (String component : recipeComponent.recipeComponents) {
                ...
                String type = split[1];
                recipe.addItemCraftBehaviour(new ConsumeItemCraftBehaviour(new CraftInHandIngredientPredicate(type),
                        count, PlayerInventorySlotResolver.singleton()));
            }
        }
        ...
        if (recipeComponent.recipeActivators != null) {
            for (String activator : recipeComponent.recipeActivators) {
                ...
                int count = Integer.parseInt(split[0]);
                String type = split[1];
                recipe.addItemCraftBehaviour(new PresenceItemCraftBehaviour(new CraftInHandIngredientPredicate(type),
                        count, PlayerInventorySlotResolver.singleton()));
            }
        }
    ...
    private final class StationTypeFilter implements Predicate<EntityRef> {
        ...
        public boolean apply(EntityRef entity) {
            CraftingStationMaterialComponent stationMaterial =
                    entity.getComponent(CraftingStationMaterialComponent.class);
